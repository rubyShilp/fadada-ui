<template>
  <span>
    <transition
      ...
      <div
        class="fa-popover fa-popper"
        :class="[popperClass, content && 'fa-popover--plain']"
        ref="popper"
      ...
      >
        <div class="fa-popover__title" v-if="title" v-text="title"></div>
        <slot>{{ content }}</slot>
      </div>
    </transition>
...
export default {
  ...
  mounted() {
    ...
    // 可访问性
    if (reference) {
      addClass(reference, 'fa-popover__reference');
